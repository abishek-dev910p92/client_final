...
=======
    ...
    document.getElementById('cid').textContent = dbUser.cid;
    document.getElementById('first_name').textContent = dbUser.first_name;
>>>>>>> 32dd4408e363c9e3055badf88ac4752ff7840803
...
function decreaseQuantity(productId) {
  ...
  if (productIndex !== -1 && checkoutProducts[productIndex].quantity > 1) {
    checkoutProducts[productIndex].quantity--;
    updateCheckoutProductCount();
    updateQuantityField();
    displayCheckoutProducts();
...
function displayProducts(products) {
  ...
  products.forEach((product) => {
    products_html += `
      <div class='col-xl-3 col-md-6 mb-xl-0 mb-4 my-2'>
        <div class='card card-blog card-plain rounded rounded-2 shadow shadow-2 p-3'>
          <div class='position-relative'>
            <a class='d-block border border-radius-xl d-flex justify-content-center mx-1'>
              <img src='${product.product_image1}' alt='img-blur-shadow' class='img-fluid fixed-size-image border-radius-xl p-1'>
            </a>
          </div>
          <div class='card-body px-1 pb-0'>
            <p class='text-gradient text-dark mb-2 text-sm'>${product.category}</p>
            <a href='javascript:;'>
              <h5>Tittle: ${product.product_title}</h5>
              <h6 class='text-sm'>Pcode: ${product.pcode}</h6>
            </a>
            <h5>Price: ${product.product_price}₹</h5>
            <p class='mb-4 text-sm'>Description: ${product.product_discription}</p>
            <div class='d-flex align-items-center justify-content-between'>
              <button type='button' class='btn btn-primary btn-sm mb-0' onclick='onAddToCheckout("${product.pid}")'>
                Add to Checkout
              </button>
            </div>
          </div>
...
function onAddToCheckout(productId) {
  fetch(`https://minitzgo.com/api/fetch_products.php`, {
    ...
    .then((data) => {
      const productToAdd = data.data.find((product) => product.pid === productId);
      if (productToAdd) {
...
function sendConfirmedBill() {
  ...
  const productsToSend = checkoutProducts.map(product => {
    return {
      ...
      totalAmount: product.quantity * product.product_price,
...
async function handleStockUpdate() {
  const enteredCode = document.getElementById("product_code_input").value.trim();
      ...
      if (matchingProduct) {
        ...
        if (quantity > currentStock) {
          ...
          const updateResponse = await fetch("https://minitzgo.com/api/stockupdatepcode.php", {
            ...
            headers: {
              "Content-Type": "application/json",
              "x-api-key": "05e335df012afa910ea6f8f02908348a715a4f0b4a2db92e113ae9b763719a1a",
            },
            body: JSON.stringify({
              pcode: enteredCode,
              stock: updatedStock,
          ...
          output = updateData.status 
            ? `Stock updated successfully. Remaining: ${updatedStock}`
            : "Failed to update stock. Please retry.";
        }
      }
    }
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (form) {
    form.addEventListener("submit", function(event) {
      ...
      fetch(`https://minitzgo.com/api/fetch_products.php`, {
        ...
        body: JSON.stringify({ cid: dbUser.cid })
      })
        .then((response) => response.json())
        .then((data) => {
  ...
  if (productCodeInput) {
    productCodeInput.addEventListener("change", handleStockUpdate);
  }

  // Bill modal listener
  const billingModal = document.getElementById("billingModal");
  if (billingModal) {
    billingModal.addEventListener("shown.bs.modal", function() {
      updateQuantityField();
      displayCheckoutProducts();
    });
  }

  // Send bill button listener
  const sendBillBtn = document.getElementById("sendConfirmedBillBtn");
  if (sendBillBtn) {
    ...
    fetch('https://minitzgo.com/api/bills_fetch.php', {
      ...
      .then(apiResponse => {
        ...
        recentBills.forEach(bill => {
          const filepath = bill.filepath?.trim() || '#';
          ...
          listItem.innerHTML = `
            ...
            <div class="d-flex align-items-center text-sm">
              <a href="${filepath}" target="_blank" rel="noopener noreferrer" 
                 ...
                 download="bill_${bill.id}.pdf">
                <i class="fas fa-file-pdf text-lg me-1"></i> View/Download
              </a>
            </div>
          `;
  ...
  if (dbUser) {
    ...
    if (dbUser.cid) {
      fetch('https://minitzgo.com/api/client_bill_info.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
